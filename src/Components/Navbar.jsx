...
const Navbar = () => {
  const [showMoblileMenu,setShowMobileMenu] = useState(false);
 useEffect(() => {
   if(showMoblileMenu)
   {
...
document.body.style.overflow = 'auto'
  ...
  return (
    <div className="absolute top-0 left-0 w-full z-10">
      ...
      {/* mobile menu  */}
      <div className={`md:hidden ${showMoblileMenu ? 'fixed w-full' : 'h-0 w-0'}  right-0 top-0 bottom-0 overflow-hidden bg-white transition-all`}>
        ...
        <ul className="flex flex-col items-center gap-2 mt-5 mx-5 text-lg font-medium ">
          ...
          <a onClick={() =>setShowMobileMenu(false)} href="#Testimonials" className="cursor-pointer hover:text-gray-400">
            Testimonials
          </a>
        </ul>
      </div>
